...
#--------------------------------------------------------------------------------------------------
# in case pw0 is present remove it

#directory='D:\\OneDrive - University of Idaho\\lt_runs_soils_11_09_2020\\'
#for parent, dirnames, filenames in os.walk(directory):
#    for fn in filenames:
#        if fn.lower().endswith('pw0.sol'):
#            #print(os.path.join(parent, fn))
#            os.remove(os.path.join(parent, fn))

#--------------------------------------------------------------------------------------------------
#Watershed Directory containing all the scenario subdirs
dir = "D:\\OneDrive - University of Idaho\\lt_runs_soils_11_09_2020\\40_Edgewood_Creek\\"
...
def get_mukey_str(a):
     with open(a,'rt') as f:
        file = f.readlines()
        for line in file:
            if line.__contains__('Mukey'):
                mukey = int(line.strip().rsplit(':', 1)[-1])
                return(mukey)
...
def get_mukey(a):
     with open(a,'r') as f:
        ...
        mukey = int(file[6].strip().rsplit(':', 1)[-1])
        return(mukey)
...
for path in  range(len(subdirpath)):
    ...
    for i in range(len(soils_list)):
        ...
        if not  any(["Water", "Urban"]) in x.values():
            mukey = get_mukey_str(d)
        elif "Water" in x.values():
            mukey = 999999
        else:
            mukey= 999991
